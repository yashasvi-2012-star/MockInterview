...
export default function InterviewHistory() {
  const { data = [], error, isError, isLoading } = useApiQuery(['interview-history'], interviewService.getHistory);
            ...
            <tbody className="divide-y divide-slate-100">
              {data.length ? data.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>{item.duration}</td>
                  <td><Badge tone={item.score >= 80 ? 'green' : 'amber'}>{item.score || 0}%</Badge></td>
                </tr>
              )) : (
                <tr>
